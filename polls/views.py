...
def index3(request):
    ...
    orders = 150
    bounce_rate = 53
    monthly_sales = [100, 200, 300, 400, 500]
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('index3')  # 'index2' olarak adlandırılmış bir sayfaya yönlendir
        else:
...
def add_order(request):
    if request.method == 'POST':
        order_name = request.POST.get('order_name')  # Formdan gelen veriyi al
        if order_name:
            # Yeni bir sipariş oluştur
            Orders.objects.create(name=order_name)
        return redirect('index3')  # Başarıyla ekledikten sonra index2 sayfasına dön
    return redirect('index3')  # Eğer POST değilse, yine index2'ye yönlendir
